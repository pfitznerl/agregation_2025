        ...
        for(iech in 1:length(liste_ech)) {
            ...
            for (iagreg1 in 1:length(liste_agreg)){
                ...
                for (iagreg2 in 1:length(liste_agreg)) {
                    ...
                    if(length(icol1)>0 & length(icol2)>0) e2=e2[idNA1] else e2=rep(NA,1253)#on supprime les lignes correspondant aux NA de e1, même si on devrait avoir idNA1=idNA2 normalement
