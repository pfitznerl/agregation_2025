#pour ne pas lancer sur un noeud de login !!!!
hostName=Sys.info()[4]
if (substr(hostName, 1,12)=="belenoslogin") stop("!!!!!!!!!!!!!!!! tu es sur une noeud de login !!!!!!!!!!!!!!!!!!!!!")
if (substr(hostName, 1,12)=="taranislogin") stop("!!!!!!!!!!!!!!!! tu es sur une noeud de login !!!!!!!!!!!!!!!!!!!!!")
...
#on prend le max de processus disponible pour R, en particulier pour rfsrc, mais cela semble fonctionner sans aussi...
...
#$$registerDoMC(120)#number of cores for the parallelisation
...
#lunch of the agregations for every sliding window (parallelized over the windows)
#trace rmse et parametres pour chacune des stations en fonction de la fenêtre glissante,
#et renvoie une liste contenant la somme sur les stations des rmse
lance_station<-function(agreg,file_station, liste_window, X, Y, T, date.valid, start.date,end.date){
  ...
  ech=strsplit(strsplit(liste_station[1], "/")[[1]][3],"_")[[1]][4] #echeance traitee
  ...
  predictions.ech.agreg.sta=mylist <- sapply(paste("W",liste_window,sep=""),function(x) NULL)#vecteur qui contiendra les previsions de l'aggregation pour chacune des fenêtres
  ...
  registerDoMC(min(n.window,100)) #fixe un nombre de processus

  #on peut enlever randomForestSRC
  out_paralel=foreach(iw=1:n.window,.inorder = TRUE, .packages=c("foreach","doMC","parallel","randomForestSRC"))%dopar%{#parallelisation over the windows, a la fin rassemble les sorties dans une liste out_paralel
  #out_paralel=foreach(iw=1:n.window,.inorder = TRUE)%do%{#parallelisation over the windows, a la fin rassemble les sorties dans une liste out_paralel
  #for (iw in 1:n.window){ print(" ATTENTION parallelisation window desactivee !!!!!!")
    rmse.ech.agreg.sta.window=NULL
    window=liste_window[iw]
    #out : liste contenant le vecteur de toutes les predictions et les poids qui ont fait toutes ces previsions et l'objet mixture correspondant
    if (agreg=="UNIFORM"){out=prevision_UNIFORM(X,Y,T)}
    if (agreg=="BOA"){out=prevision_BOA(X,Y,window,T,sta,ech,date.valid)} #date.valid necessaire pour la fonction getFirstLeadTime()
    if (agreg=="SOCOBOA"){out=prevision_SOCOBOA(X,Y,window,T)}
    if (agreg=="FS"){out=prevision_FS(X,Y,window,T,sta,ech)}
    if (agreg=="FSCALIB"){out=prevision_FSCALIB(X,Y,window,T,sta,ech)}
    if (agreg=="FSBOA"){out=prevision_FSBOA(X,Y,window,T,sta,ech)}
    if (agreg=="FSSOCOBOA"){out=prevision_FSSOCOBOA(X,Y,window,T)}
    if (agreg=="FSBOACALIB"){out=prevision_FSBOACALIB(X,Y,window,T)}
    if (agreg=="FSSOCOBOACALIB"){out=prevision_FSSOCOBOACALIB(X,Y,window,T)}
    if (agreg=="SMH"){out=prevision_SMH(X,Y,window,T)}
    if (agreg=="SMHCALIB"){out=prevision_SMHCALIB(X,Y,window,T)}
    if (agreg=="SMBOA"){out=prevision_SMBOA(X,Y,window,T)}
    if (agreg=="SMBOACALIB"){out=prevision_SMBOACALIB(X,Y,window,T)}
    if (agreg=="EWA"){out=prevision_EWA(X,Y,window,T,sta,ech)}
    if (agreg=="NBM"){out=prevision_NBM(X,Y,window,T,sta,ech)}
    if (agreg=="EWACALIB"){out=prevision_EWACALIB(X,Y,window,T)}
    if (agreg=="MLpol"){out=prevision_MLpol(X,Y,window,T,sta,ech)}
    if (agreg=="MMLpol"){out=prevision_MMLpol(X,Y,window,T,sta,ech)}
    if (agreg=="MLprod"){out=prevision_MLprod(X,Y,window,T,sta,ech)}
    if (agreg=="MMLprod"){out=prevision_MMLprod(X,Y,window,T,sta,ech)}
    if (agreg=="META"){out=prevision_META(X,Y,3,T,sta,ech,"BOA",c("BOA","BOA","BOA","BOA"),c(window,100,14,3))}
    ...
    if(agreg=="bestShift") {
      ...
      stop("pred n'est pas disponible pour bestShifting, il y aura donc des nan pour bestshift, pour quand meme avoir le rmse, on peut
                bidouiller une des agregation et la faire prevoir comme le meilleur expert en regardant min(lexp),
                on peut  même mettre les poids à zero et 1 si on veut voir les poids...")
    }
    if(agreg=="bestExpert") {
      out=NULL
  ...
  if (flag.write.txt) {
    for (iw in 1:n.window){ #for each window
      ...
      write.table(df_write,paste(dir_exe,dir_sorties,name_save,"_",agreg,"_",sta,"_",
                  liste_window[iw],"_",ech,".txt",sep=""), sep=";",col.names = TRUE, row.names = FALSE)

      # colnames(df_write)=c("Y",agreg,names.experts,paste("weights_",names.experts,sep=""))
      # write.table(df_write,paste(dir_exe,dir_sorties,"sleep_avec_kf_avec_FTL_",agreg,"_",sta,"_",
      #             liste_window[iw],"_",ech,".txt",sep=""), sep=";",col.names = TRUE, row.names = FALSE)

      # colnames(df_write)=c("Y",agreg,names.experts,paste("weights_",names.experts,sep=""))
      # write.table(df_write,paste(dir_exe,dir_sorties,"sleep_avec_kf_sans_FTL_",agreg,"_",sta,"_",
      #             liste_window[iw],"_",ech,".txt",sep=""), sep=";",col.names = TRUE, row.names = FALSE)

      # colnames(df_write)=c("Y",agreg,names.experts,paste("weights_",names.experts,sep=""))
      # write.table(df_write,paste(dir_exe,dir_sorties,"sleep",agreg,"_",sta,"_",
      #             liste_window[iw],"_",ech,".txt",sep=""), sep=";",col.names = TRUE, row.names = FALSE)
      
      # colnames(df_write)=c("Y",agreg,names.experts,paste("weights_",names.experts,sep=""))
      # write.table(df_write,paste(dir_exe,dir_sorties,agreg,"FTL_",sta,"_",
      #               liste_window[iw],"_",ech,".txt",sep=""), sep=";",col.names = TRUE, row.names = FALSE)
    }
  }

  #on stocke les donnees associees aux fenêtres dans des vecteurs et listes
  for (iw in 1:n.window){ #for each window
    window=liste_window[iw]
  ...
  #quantile of the agregation's absolute error for a station, a lead time and a window
  if(flag.quantiles){
    for (iw in 1:n.window){
      window=liste_window[iw]
      quantile_abs_error=eval_quantiles_abs_error(predictions.ech.agreg.sta[[iw]],Y)
      #cat to print in the file :
      cat("\n quantiles pour l'agregation ",agreg," station ",sta," echeance ",ech," fenêtre \n",file=file.out,append=T)
  ...
  if (flag.plot.ech.sta.window){
    for (iw in 1:n.window){ #for each window, plot of ...
      ...
      #png(paste(dir_plot,dir_weights,"weights_",agreg,"_window",liste_window[iw],"_ech",ech,"_",sta,"_",start.date,"_",end.date,".png",sep=""),width = 10000, height = 1000)
      #$$legend("topleft", legend = colnames(X), col=1:4, pch=1)
      #matplot(weights.ech.agreg.sta[[iw]], type = c("b"),pch=1,col = 1:7)
      print(names.experts)
      lance_plot_weights(weights.ech.agreg.sta[[iw]],names.experts,plot_name,date.valid,plot.periode,T)

      #plot de l'object mixture
      if (agreg!="UNIFORM") {
      ...
      #plot des obs et des previsions
...
lance_agreg <- function(agreg){
  #$$ alpha.tot=NULL #vector which will contain all the alpha of every station for each window
  #$$ eta.tot=NULL #vector which will contain all the eta of every station for each window
  ...
  #for (ista in 1:n.sta){ print(" ATTENTION parallelisation station desactivee !!!!!!")
    file_station=liste_station[ista]
    sta=substr(strsplit(file_station, "_")[[1]][7],1,8) #insee of the station
    ech=strsplit(strsplit(liste_station[1], "/")[[1]][3],"_")[[1]][4] #echeance traitee
    cat("\n")
    cat(paste("######################################## ech : ",ech,", agreg : ",agreg,", station : ",sta,"########################################\n"))
    cat("        temperature agregation for all the sliding windows:\n")
    ...
    nbr_exp=ncol(X)
    T=length(Y)
    # liste_window=seq(60,T,60)
    # liste_window=c(100,T)#$$
    #liste_window=c(60,100,500,T)
    liste_window=c(seq(60,360,60),seq(400,1100,100),T)
    liste_window=c(seq(60,420,60),520,620,720,seq(780,1120,60),T)
    liste_window=c(seq(60,1200,60),T)
    liste_window=c(T)

    n.window=length(liste_window)

    #lunch the agregation for one agregation, one lead time and one station
    liste=lance_station(agreg,file_station, liste_window, X ,Y, T, date.valid,start.date, end.date)#liste dont les elements sont les donnees associees aux fenêtres pour une station
  ...
  all_obs<-unlist(obs.sta) #pour iech, on concatene toutes les obs de toutes les stations dans un vecteur
  for (iw in 1:n.window) {
    window=liste_window[iw]
    ...
    print(paste("skill scores pour toutes les stations",agreg, ", ech : ",ech,", window : ",window,
    ...
    cat("\n")
    print(paste("rmse pour toutes les stations, de l'agregation ",agreg,", echeance ",ech,", fenêtre ",window," RMSE : ",round(rmse.agreg.ech.window,3),sep=""))
    cat("\n")
    cat("\n rmse pour toutes les stations, de l'agregation ",agreg,", echeance ",ech,", fenêtre ",window,file=file.out,append=T)
    cat(round(rmse.agreg.ech.window,3),"\n",file=file.out,append=T)
    if (flag.quantiles){#quantiles over all the stations of the agregation's absolute error for one window and one lead time
      quantile_abs_error=eval_quantiles_abs_error(all_iw_predictions,all_obs)
      cat("\n",file=file.out,append=T)
      cat("quantiles pour toutes les stations, de l'agregation ",agreg,", echeance ",ech,", fenêtre \n",window,file=file.out,append=T)
...
#recover the experts predictions and the obsevations of the data and the start and end date of the data  
recup_data <-function(file_station){
    #on recupere les donnees
    donnee = read.table(file_station,head=T,sep=";",na.strings=c("       NA","      NA","     NA","    NA","   NA","  NA"," NA","NA")) #il y a un ou des espaces avant les NA..., d'ou "  NA"
    names(donnee)[which(names(donnee)=="Q20")]="Q30" # TODO verrue !!!!!! it's the Q30 and not the Q20, it would be better to change this in the code on sotrtm35-sidev which creates the .txt
    
    #verrue pour gérer lorsqu'on prend donnees dans dir_data_without
    ...
    #donnee=donnee[1:730,]
    #donnee=donnee[1:150,]
    #donnee=donnee[1254:nrow(donnee),]
    donnee=donnee[1:1253,]
    # donnee=donnee[1254:1772,] #1:1253 ou 1254:1772 519 obs training
    # donnee=donnee[1:250,]

    #$$ TODO verifier qu'on gere bien les -100 issus de la bdpe

    #$$ TODO verifier que tout est ok , il ne faut pas remplacer par 0 sinon pb division par zero dans Opera_dev.R
    ...
    # print("on met des experts specialise")
    # X_spe=subset(X, select = c(Q10,Q30,Q70,Q90))
    # colnames(X_spe)=paste("spe_",colnames(subset(X, select = c(Q10,Q30,Q70,Q90))),sep="")
    # X=cbind(X,X_spe)
...
# plot.periode=c("2020-03-30","2023-03-01") # avec pearp
# plot.periode=c("2018-06-01","2021-05-09") # sans pearp
# plot.periode=c("2020-12-15","2021-02-01") # pour evenement de Lyon mi janvier 2021
# plot.periode=c("2021-12-30","2022-02-01") # pour evenement de Lyon mi janvier 2022
# plot.periode=c("2023-04-20","2023-05-20") # pour evenement de Chamonix mai 2023
# plot.periode=c("2021-12-01","2022-02-01") # pour evenement de Chamonix juste avant Noël 2021
# plot.periode=c("2021-12-09","2021-12-29")
plot.periode=c("2021-12-09","2021-12-29") # pour evenement de Chamonix juste avant Noël 2021 PAPIER
# plot.periode=c("2023-01-15","2023-03-03") # pour evenement de Chamonix mi janvier et debut fevrier
# plot.periode=c("2023-01-10","2023-02-05") # pour evenement de Chamonix mi janvier
# plot.periode=c("2023-02-07","2023-03-07") # pour evenement de Chamonix debut fevrier
# plot.periode=c("2022-10-10","2022-11-15") # pour evenement de Chamonix fort biais positif puis negatif
# plot.periode=c("2020-03-30","2020-04-18") # pour evenement (tout au debut des données) de Lyon echeance 6h
# plot.periode=c("2023-05-25","2023-06-05") # pour evenement de 83031001 ech 15
# plot.periode=c("2021-01-20","2021-02-10") # pour evenement de 83031001 ech 30
# plot.periode=c("2023-12-15","2024-01-25")
# plot.periode=c("2020-03-30","2020-04-01") # pour ne pas planter avec seulement 250 iterations
...
liste_ech_xgboost=liste_ech #necessaire pour la fonction recupLeadTimes

#liste_ech=c("6","9","12")
#liste_ech=c("15","18","21")
#liste_ech=c("24","27","30")
#liste_ech=c("33","36","39")
#liste_ech=c("42","45","48")
#liste_ech=c("57","72","84")
#liste_ech=c("9")
...
liste_agreg <- c("UNIFORM","BOA","MLpol","MMLpol","MLprod","MMLprod","FSBOA","SOCOBOA","FSSOCOBOA","FSSOCOBOACALIB","SMBOACALIB","SMBOA","FSBOACALIB","FS","FSCALIB",
                  "SMH","SMHCALIB","SMBOA","SMBOACALIB","EWA","EWACALIB","bestConvex","bestShift","bestExpert")
liste_agreg <- c("BOA","FSBOACALIB","FSCALIB","SMHCALIB")
liste_agreg <- c("BOA","SMBOA","SMBOACALIB","FSBOACALIB","FSBOA","SMHCALIB","FSCALIB","FS")
liste_agreg <- c("BOA","SMBOA","SMBOACALIB","FSBOACALIB","FSBOA","SMHCALIB","SMH","FSCALIB","FS")
liste_agreg <- c("FS","FSCALIB","FSBOACALIB","FSBOA","EWACALIB","BOA")
#liste_agreg <- c("FSCALIB")
liste_agreg <- c("bestConvex")
liste_agreg <- c("BOA","EWA","FS","FSBOA")
liste_agreg <- c("BOA","EWA","MLpol")
...
file.out=paste("results.txt",sep="") #file in which will be written the prints and scores
name_save=strsplit(file.out,".txt")[[1]][1]
#file.out=paste("results.txt",sep="") #file in which will be written the prints and scores
...
for (iech in 1:n.ech){
  ...
  # #££
  # # liste_files_train=list.files(paste("/scratch/work/pfitznerl/training_agreg/allEch",ech,sep=""),full.names = TRUE)
  # # which(validH %in% seq(18,echH[length(echH)],24))
  # # liste_train=list()
  # # for (itrain in 1:length(liste_files_train)) {
  # #   file=liste_files_train[itrain]
  # #   liste_train[[itrain]]=read.table(file,header=TRUE,sep=";")
  # # }
  # file_train=list.files(paste("/scratch/work/pfitznerl/training_agreg/allStaEch",sep=""),full.names = TRUE,patter="1253")
  # liste_ech_train=liste_ech_xgboost[ which( liste_ech_xgboost %in% seq(as.numeric(ech)%%24,120,24))] #same day hour
  # liste_ech_train=c(liste_ech_train,liste_ech_xgboost[which(liste_ech_xgboost==(as.numeric(ech)-3))],liste_ech_xgboost[which(liste_ech_xgboost==(as.numeric(ech)+3))]) #lead time before and after
  # liste_train=list()
  # data_training_all <- read.table(file_train, header = TRUE, sep = ";", na.strings = "NA", dec = ".") #training data, all iterations, all stations all lead times
  # # data_training_all=data_training_all[which(data_training_all$ech %in% liste_ech_train),]
  # source("transition_learning_xgboost_multiple_ech_sta.R")
  # #££
  ...
  #liste_station=dir(path = dir_data_without, pattern = paste("donnee_",parametre,"_",res,"_",ech,"_",sep=""),full.names=TRUE)
  liste_station=dir(path = dir_data_with, pattern = paste("donnee_",parametre,"_",res,"_",ech,"_",sep=""),full.names=TRUE)
  #liste_station=dir(path = dir_data_with, pattern = paste("donnee_",parametre,"_",res,"_",ech,"_",sep=""),full.names=TRUE)[c(25)]
  #liste_station=dir(path = dir_data_with, pattern = paste("donnee_",parametre,"_",res,"_",ech,"_",sep=""),full.names=TRUE)[c(7)]
  #liste_station=dir(path = dir_data_with, pattern = paste("donnee_",parametre,"_",res,"_",ech,"_",sep=""),full.names=TRUE)[-c(26,31,32,33)]
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_14137001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_06088001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_69029001.txt",sep="")
  liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_74056001.txt",sep="")
  # liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_73054001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_74056405.txt",sep="")
  # liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_31069001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_68297001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_87085006.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_58160001.txt",sep="")
  #liste_station=c(paste(dir_data_with,"/donnee_t_00_",ech,"_31069001.txt",sep=""),paste(dir_data_with,"/donnee_t_00_",ech,"_33281001.txt",sep=""))
  #                ,paste(dir_data_with,"/donnee_t_00_",ech,"_58160001.txt",sep=""),paste(dir_data_with,"/donnee_t_00_",ech,"_74056001.txt",sep=""))
  #liste_station=c(paste(dir_data_with,"/donnee_t_00_",ech,"_31069001.txt",sep=""),paste(dir_data_with,"/donnee_t_00_",ech,"_74056405.txt",sep=""))
  #liste_station=c(paste(dir_data_with,"/donnee_t_00_",ech,"_31069001.txt",sep=""),paste(dir_data_with,"/donnee_t_00_",ech,"_74056001.txt",sep=""))
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_58160001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_84087001.txt",sep="")
  #liste_station=dir(path = dir_data_with, pattern = paste("donnee_",parametre,"_",res,"_",ech,"_",sep=""),full.names=TRUE)[6]
  #liste_station=dir(path = dir_data_with, pattern = paste("donnee_",parametre,"_",res,"_",ech,"_",sep=""),full.names=TRUE)[c(26,29,33)]
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_83031001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_67124001.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_74056405.txt",sep="")
  #liste_station=paste(dir_data_with,"/donnee_t_00_",ech,"_75114001.txt",sep="")
...
if (flag.save){
  nameRdata=paste(name_save,".Rdata",sep="") 
...
if (flag.save){
  nameRdata=paste(name_save,".Rdata",sep="")
  save.image(nameRdata)
  #system(paste("mv ",nameRdata," ",dir_utemp,sep=""))
}

end_time <- Sys.time()
runTime=end_time - start_time
print(paste("run time : ",runTime,sep=""))
