prevision_BOA <-function(X,Y,window,T,sta,ech,date.valid){#fonction qui fait la prevision par BOA avec la fenêtre glissante window, retourne la liste des previsions
    ...
    data_BOA$training$sta=sta #$$ needed to train the transitionObjects on the other lead times
    data_BOA$training$ech=ech #$$ needed to train the transitionObjects on the other lead times
    for (t in seq(1,T)){
        data_BOA$training$date.valid=date.valid[t]
        data_BOA$training$iteration=t #$$ needed to train the transitionObjects on the other lead times
        if(data_BOA$T>(window -1)){
            #stop("si sleeping BOA, il faut mettre les ref_XXX de sleeping boa dans fonction_prevision")
            ...
            #data_BOA$weights=data_BOA$weights[2:window,] we want all the weights for the plot
            ...
            #ref_XXX pour BOA reference pour reveiller les experts
            data_BOA$training$ref_R=data_BOA$training$ref_R-data_BOA$training$ref_r[1,]
            data_BOA$training$ref_R.reg=data_BOA$training$ref_R.reg-data_BOA$training$ref_r.reg[1,]
            data_BOA$training$ref_V=data_BOA$training$ref_V-data_BOA$training$ref_r[1,]^2
            data_BOA$training$ref_V[which(data_BOA$training$ref_V<0)]=0
            data_BOA$parameters$ref_eta=data_BOA$parameters$ref_eta[2:window,]  	
            data_BOA$training$ref_r=data_BOA$training$ref_r[2:window,]
            data_BOA$training$ref_r.reg=data_BOA$training$ref_r.reg[2:window,]
            data_BOA$training$all_R=data_BOA$training$all_R-data_BOA$training$all_r[1,]
            data_BOA$training$all_R.reg=data_BOA$training$all_R.reg-data_BOA$training$all_r.reg[1,]
            data_BOA$training$all_V=data_BOA$training$all_V-2.2*data_BOA$training$all_r[1,]^2
            data_BOA$training$all_V[which(data_BOA$training$all_V<0)]=0
            data_BOA$parameters$all_eta=data_BOA$parameters$all_eta[2:window,]  	
            data_BOA$training$all_r=data_BOA$training$all_r[2:window,]
            data_BOA$training$all_r.reg=data_BOA$training$all_r.reg[2:window,]
            data_BOA$training$FTL=data_BOA$training$FTL[2:window,]
            data_BOA$training$experts=data_BOA$training$experts[2:window,]
            data_BOA$training$y=data_BOA$training$y[2:window,]

            data_BOA$T=window-1
            data_BOA$loss=mean(loss(c(data_BOA$prediction), c(data_BOA$Y), loss.type = data_BOA$loss.type))
        }
        #data_BOA = predict(data_BOA,newexpert=X,newY=Y,online=TRUE,awake=awake,parameters=parameters)
        #if (t==579){browser()}
        ...
        #matrix_weights=rbind(matrix_weights,c(data_BOA$coefficients)) #on ajoute les derniers poids de l'agregation
        liste_eta=rbind(liste_eta,data_BOA$parameters$eta[nrow(data_BOA$parameters$eta),])
    }
    # nameR=paste("data_creation/",sta,"_BOA_",ech,".RData",sep="")
    # dataToSave=data_BOA$training$dataToSave #save() can only save "entire" objects and not one column of an object like object$toto
    # save(dataToSave,file=nameR)
    # nameR=paste("/scratch/work/pfitznerl/aggregation/shapleyValues/shapleyValues_",sta,"_BOA_",ech,".RData",sep="")
    # shapleyValues=data_BOA$training$shapleyValues #save() can only save "entire" objects and not one column of an object like object$toto
    # xgb_model=data_BOA$training$transObject_threshold
    # save(shapleyValues,xgb_model,file=nameR)
    ...
    BOA$weights=as.matrix(data_BOA$weights) #as.matrix(matrix_weights)
    ...
    BOA$awake_transition=data_BOA$awake_transition #$$pour faire le boa awake avec transition
    ...
    GSS=(PSS1+PSS2)/2
    # print(paste("BOA, sta : ",sta,", ech : ",ech,", window : ",window,
    ...
    #             ", GSS : ",GSS,
    #             ", NERR (nb wrong experts awake) : ",NERR,sep=""))
