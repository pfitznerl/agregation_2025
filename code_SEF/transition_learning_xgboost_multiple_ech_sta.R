...
update.probTransition <- function(transObject,new_training_data=NULL,loss=NULL,threshold,xgb_training=NULL){
    ...
    if(predType=="loss") {
        ...
        transObject <- modelpredErrUpdate(transObject,new_training_data,loss,threshold,xgb_training)
    } else {
...
modelOrderUpdate <- function(transObject,new_training_data) {
    ...
    if (modelType=="rf") {
        #when using get.mv.formula, we don't need "(y1,y2,y3)~."
        # transObject$model=rfsrc(get.mv.formula(colnames(transObject$training_data[,colPred])),
        #     data = transObject$training_data, na.action = "na.omit")
...
modelpredErrUpdate <- function(transObject,new_training_data,loss,threshold,xgb_training) {
    ...
    modelType=transObject$modelType
    minTrain=transObject$minTrain
    if(!is.null(xgb_training)) {ndata=nrow(xgb_training)} else ndata=0 #number of observations

    transObject$training_data=xgb_training
    if (ndata>=nTrain) transObject$training_data=tail(transObject$training_data,n=nTrain) #we only keep nTrain data
    ...
    if(ndata>minTrain-10){#to train the RF before we make a prediction
        ...
        if (modelType=="rf") {
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            #POUR OPER LORSQU'ON CONNAIT LES HYPERPARAMETRES
            #carrefull: xgb.train and not just train to avoid conflict with train() function of other libraries
            # transObject$model=xgb.train(data=dtrain,max_depth = XXX,
            #    eta = XXX, nthread = XXX, nrounds = XXX, objective = "reg:squarederror")
            #POUR OPER LORSQU'ON CONNAIT LES HYPERPARAMETRES
            #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

            # # results_2.51_
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 8,gamma=0, min_child_weight=25,
            #    eta = 1, nthread = 1, nrounds = 3, colsample_bynode = 0.85, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_deux_thresholds_          zPSS_leaves_avec_kf_new_732
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 9,gamma=0, min_child_weight=10,
            #    eta = 1, nthread = 1, nrounds = 5, colsample_bynode = 0.8, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_Q95_Hyperparametre_threshold_Experts_      zPSS_leaves_avec_kf_new_732_25
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 7,gamma=0, min_child_weight=20,
            #    eta = 0.99, nthread = 1, nrounds = 3, colsample_bynode = 0.7, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_35_   oversample2   zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 6,gamma=0, min_child_weight=15,
            #    eta = 1, nthread = 1, nrounds = 7, colsample_bynode = 0.8, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_3_   oversample2   zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 8,gamma=0, min_child_weight=5,
            #    eta = 0.99, nthread = 1, nrounds = 7, colsample_bynode = 0.65, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_25_   oversample2   zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 7,gamma=0, min_child_weight=10,
            #    eta = 1, nthread = 1, nrounds = 7, colsample_bynode = 0.75, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_2_   oversample2   zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 10,gamma=0, min_child_weight=15,
            #    eta = 0.99, nthread = 1, nrounds = 7, colsample_bynode = 0.85, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_15_   oversample2   zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 9,gamma=0, min_child_weight=20,
            #    eta = 0.98, nthread = 1, nrounds = 3, colsample_bynode = 0.65, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1_   oversample2   zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 9,gamma=0, min_child_weight=35,
            #    eta = 1, nthread = 1, nrounds = 1, colsample_bynode = 0.8, subsample=1, objective = "reg:squarederror",verbose = 0)

            # results_05_    oversample2  zSAVE_PSS_leaves_avec_kf_new_519
            transObject$model=xgboost(data=training_data,max_depth = 4,gamma=0, min_child_weight=35,
               eta = 0.5, nthread = 1, nrounds = 7, colsample_bynode = 0.65, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_0_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 4,gamma=0, min_child_weight=10,
            #    eta = 0.3, nthread = 1, nrounds = 7, colsample_bynode = 0.8, subsample=1, objective = "reg:squarederror",verbose = 0)




            # # results_1253_threshold25_oversample5_35_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 4,gamma=0, min_child_weight=15,
            #    eta = 1, nthread = 1, nrounds = 7, colsample_bynode = 0.75, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1253_threshold25_oversample5_3_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 5,gamma=0, min_child_weight=15,
            #    eta = 1, nthread = 1, nrounds = 7, colsample_bynode = 0.8, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1253_threshold25_oversample5_25_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 8,gamma=0, min_child_weight=25,
            #    eta = 1, nthread = 1, nrounds = 2, colsample_bynode = 0.75, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1253_threshold25_oversample5_2_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 7,gamma=0, min_child_weight=30,
            #    eta = 1, nthread = 1, nrounds = 3, colsample_bynode = 0.7, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1253_threshold25_oversample5_15_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 8,gamma=0, min_child_weight=35,
            #    eta = 0.99, nthread = 1, nrounds = 2, colsample_bynode = 0.85, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1253_threshold25_oversample5_1_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 9,gamma=0, min_child_weight=35,
            #    eta = 1, nthread = 1, nrounds = 1, colsample_bynode = 0.75, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1253_threshold25_oversample5_05_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 9,gamma=0, min_child_weight=35,
            #    eta = 0.3, nthread = 1, nrounds = 7, colsample_bynode = 0.8, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_1253_threshold25_oversample5_0_      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 4,gamma=0, min_child_weight=5,
            #    eta = 0.3, nthread = 1, nrounds = 7, colsample_bynode = 0.85, subsample=1, objective = "reg:squarederror",verbose = 0)



            # # results_w5_519_35      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 4,gamma=0, min_child_weight=10,
            #    eta = 1, nthread = 1, nrounds = 4, colsample_bynode = 0.65, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_w5_519_3      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 5,gamma=0, min_child_weight=10,
            #    eta = 1, nthread = 1, nrounds = 5, colsample_bynode = 0.65, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_w5_519_25      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 4,gamma=0, min_child_weight=20,
            #    eta = 1, nthread = 1, nrounds = 4, colsample_bynode = 0.75, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_w5_519_2      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 11,gamma=0, min_child_weight=20,
            #    eta = 1, nthread = 1, nrounds = 3, colsample_bynode = 0.65, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_w5_519_15      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 7,gamma=0, min_child_weight=40,
            #    eta = 1, nthread = 1, nrounds = 5, colsample_bynode = 0.85, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_w5_519_1      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 11,gamma=0, min_child_weight=40,
            #    eta = 1, nthread = 1, nrounds = 1, colsample_bynode = 0.7, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_w5_519_05      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 10,gamma=0, min_child_weight=80,
            #    eta = 0.4, nthread = 1, nrounds = 7, colsample_bynode = 0.75, subsample=1, objective = "reg:squarederror",verbose = 0)

            # # results_w5_519_0      zSAVE_PSS_leaves_avec_kf_new_519
            # transObject$model=xgboost(data=xTrain,label=yTrain,max_depth = 10,gamma=0, min_child_weight=80,
            #    eta = 0.2, nthread = 1, nrounds = 7, colsample_bynode = 0.7, subsample=1, objective = "reg:squarederror",verbose = 0)
